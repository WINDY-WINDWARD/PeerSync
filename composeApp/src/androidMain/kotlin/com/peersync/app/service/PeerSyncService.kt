    ...
    private fun acquireWakeLock() {
        if (wakeLock == null) {
            ...
            wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "PeerSync::ServiceWakeLock").apply {
                acquire() // Indefinite lock to keep CPU awake during active session
            }
        }
        
        if (wifiLock == null) {
            val wm = getSystemService(Context.WIFI_SERVICE) as WifiManager
            wifiLock = wm.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "PeerSync::ServiceWifiLock").apply {
                acquire()
            }
